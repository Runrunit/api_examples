...
const createDocument = async (taskId, filename) => {
  const createDocumentUrl = `${BASE_URL}/documents?task_id=${taskId}`;
  ...
  const response = await axios.post(createDocumentUrl, payload, { headers, params: { taskId } });
  ...
  try {
    ...
    await axios.post(`${BASE_URL}/documents/${document_id}/mark_as_uploaded`, {}, { headers });
  } catch (error) {
